        ...
        for (let p2 of Planet.registry) {
            if (p1 != p2) {
                if (distPlanets(p1, p2) < p1.radius + p2.radius) {
                    ...
                    let newPos = p1.m > p2.m ? [p1.pos[0], p1.pos[1]] : [p2.pos[0], p2.pos[1]];
...
function unigravPlanets(p1, p2) {
    return unigrav(p1.m, p2.m, distPlanets(p1, p2));
}
function unigrav(m1, m2, d) {
    return (UGC * m1 * m2) / (Math.pow(d, 2));
}
function distPlanets(p1, p2) {
    let [x1, y1] = p1.pos;
    let [x2, y2] = p2.pos;
...
function calculateV(planet) {
    return ([
        0 * Math.sqrt(UGC * (planet.m / Math.pow((distPlanets(Planet.registry[0], planet)), 2)) * planet.pos[0]) + planet.v[0],
        Math.sqrt(UGC * (planet.m / Math.pow((distPlanets(Planet.registry[0], planet)), 2)) * planet.pos[1]) + planet.v[1]
    ]);
}
